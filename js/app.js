...
(function () {
  ...
  var ICON_PHONE = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.7 9.73 19.79 19.79 0 01.67 5.08 2 2 0 012.65 3h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 10.64a16 16 0 006.72 6.72l.95-.95a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>';
  var ICON_EMAIL = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>';
  ...
  function populateContact(profile) {
    ...
    // WhatsApp
    if (profile.whatsapp) {
      var waHref = 'https://wa.me/' + profile.whatsapp + '?text=Hi%20Mohammed%2C%20I%20found%20your%20portfolio!';
      btns += '<a href="' + waHref + '" target="_blank" rel="noopener" class="social-link btn-icon contact-icon-wa" aria-label="WhatsApp" title="WhatsApp">' + ICON_WA + '</a>';
    }

    // Phone
    if (profile.phone) {
      btns += '<a href="tel:' + profile.phone.replace(/\s/g, '') + '" class="social-link btn-icon" aria-label="Phone" title="' + profile.phone + '">' + ICON_PHONE + '</a>';
    }

    // Email
    btns += '<a href="mailto:' + profile.email + '" class="social-link btn-icon" aria-label="Email" title="' + profile.email + '">' + ICON_EMAIL + '</a>';

    // GitHub, LinkedIn, etc.
    btns += socialLinks(profile.socials);

    socialsEl.innerHTML = btns;
  }
